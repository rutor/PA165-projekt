...
public class ShowServiceTest extends AbstractTestNGSpringContextTests {
	...
	public void testCreate() {
		assertEquals(service.create(show1), new Long(1));
		verify(dao).create(show1);
	}

	@Test
	public void testRemove() {
		service.remove(show1);
		verify(dao).remove(show1);
	}

	@Test
	public void testUpdate() {
		service.update(show2);
		verify(dao).update(show2);
	}
@Test
public void findAllByGenre() {
	when(dao.findAllByGenre(comedy)).thenReturn(Collections.singletonList(show1));
	List<Show> shows = service.findAllByGenre(comedy);
	assertEquals(shows.size(), 1);
	assertEquals(shows.get(0), show1);
}
}